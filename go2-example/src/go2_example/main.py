...
def exec_dataflow(dataflow: DataflowBuilder, build: bool, run: bool):
...
def run_dataflow(
    ...
    if tests:
        for test in tests:
            ...
            exec_dataflow(dataflow, build, run)
    else:
        print("No tests selected to run. Use --help for options.")
        # TODO: run default dataflow without tester node
